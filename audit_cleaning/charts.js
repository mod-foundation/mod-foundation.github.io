...
function makeHistogram({ container, data, field, color = '#4a90d9', title, bins = 10, fixedMin, step }) {
    ...
    const chart = new Chart(canvas, {
        ...
        options: {
            ...
            scales: {
                x: { beginAtZero: false,grid: { display: false }, ticks: { stepSize: 1, precision: 0, maxRotation: 45, font: { size: 10 } } },
...
function renderInfrastructureCharts(data) {
    ...
    // Utilities                          
    makeBarChart({  container: 'Electrical',      data, field: 'elec_condition',     colors: ELEC_CONDITION_COLORS,     labels: ELEC_CONDITION_LABELS,     title: 'Electricity lines outside the Drain'      });
    makeBarChart({  container: 'Cables',          data, field: 'cables_condition',   colors: CABLES_CONDITION_COLORS,   labels: CABLES_CONDITION_LABELS,   title: 'Cables or lines crossing the Drain'          });
    makeBarChart({  container: 'Manholes',        data, field: 'manholes_condition', colors: MANHOLES_CONDITION_COLORS, labels: MANHOLES_CONDITION_LABELS, title: 'Manholes inside the Drain'        });
}
...
const WATER_CONTAMINATION_LABELS = { 'black': 'Black/ Grey', 'continuous': 'Continuous Flow', 'solid': 'Solid Particles/ Oily Film', 'froth': 'Froth/ Foam', 'cannot': 'Cannot See' };
...
const WATER_SMELL_LABELS         = { 'cannot': 'Cannot See', 'no odour': 'No Odour', 'Less Odour': 'Less Odour','unable': 'Strong Smell (Sewage)','necessarily':'Strong Smell (Not Necessarily Sewage)' };
